...
/*
if (PHP_SAPI == 'cli' && file_exists(__DIR__.'/../../../../.env')) {
...
if ($platformsh->inRuntime()) {
    $repo_root = $platformsh->appDir; // No slash at the end.
    $site_uri = $platformsh->getPrimaryRoute()['url']; // Has slash at the end.
    if ($platformsh->hasRelationship('database')) {
        $creds = $platformsh->credentials('database');
        $cms_database_user = $creds['username'];
        $cms_database_password = $creds['password'];
        $cms_database_name = $creds['path'];
        $cms_database_host = $creds['host'];
        $cms_database_port = $creds['port'];
        $civicrm_database_user = $creds['username'];
        $civicrm_database_password = $creds['password'];
        $civicrm_database_name = 'civicrm';
        $civicrm_database_host = $creds['host'];
        $civicrm_database_port = $creds['port'];
    }
    // No Platform database relationship.
    else {

    }
}
// Not in platform runtime. Set local variables.
else {
    $repo_root = getenv('REPO_ROOT');
    $site_uri = getenv('SITE_URI');
    $cms_database_user = getenv('DATABASE_USER');
    $cms_database_password = getenv('DATABASE_PASSWORD');
    $cms_database_name = getenv('WP_DATABASE');
    $cms_database_host = getenv('DATABASE_HOST');
    $cms_database_port = getenv('DATABASE_PORT');
    $civicrm_database_user = getenv('DATABASE_USER');
    $civicrm_database_password = getenv('DATABASE_PASSWORD');;
    $civicrm_database_name = getenv('CIVICRM_DATABASE');;
    $civicrm_database_host = getenv('DATABASE_HOST_CIVI');
    $civicrm_database_port = getenv('DATABASE_PORT');
}
...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
  define( 'CIVICRM_UF_DSN', 'mysql://' . $cms_database_user . ':' . $cms_database_password . '@' . $cms_database_host . ':' . $cms_database_port . '/' . $cms_database_name . '?new_link=true');
}
...
if (!defined('CIVICRM_DSN')) {
  if (CIVICRM_UF === 'UnitTests' && isset($GLOBALS['_CV']['TEST_DB_DSN'])) {
    ...
    define('CIVICRM_DSN', 'mysql://' . $civicrm_database_user . ':' . $civicrm_database_password . '@' . $civicrm_database_host . ':' . $civicrm_database_port . '/' . $civicrm_database_name . '?new_link=true');
  }
}
...
if (!defined('CIVICRM_TEMPLATE_COMPILEDIR')) {
  define( 'CIVICRM_TEMPLATE_COMPILEDIR', $repo_root . '/web/wp-content/uploads/civicrm/templates_c/');
}
...
$civicrm_setting['Directory Preferences']['uploadDir'] = $repo_root . '/web/wp-content/uploads/civicrm/upload/';
$civicrm_setting['Directory Preferences']['imageUploadDir'] = $repo_root . '/web/wp-content/uploads/civicrm/persist/contribute/';
$civicrm_setting['Directory Preferences']['customFileUploadDir'] = $repo_root . '/web/wp-content/uploads/civicrm/custom/';
$civicrm_setting['Directory Preferences']['customTemplateDir'] = $repo_root . '/web/wp-content/civicrm/templates/';
$civicrm_setting['Directory Preferences']['customPHPPathDir'] = $repo_root . '/web/wp-content/civicrm/php/';
$civicrm_setting['Directory Preferences']['extensionsDir'] = $repo_root . '/web/wp-content/civicrm/extensions/';
$civicrm_setting['URL Preferences']['userFrameworkResourceURL'] =  $site_uri . '/wp-content/plugins/contrib/civicrm/civicrm/core/';
...
$civicrm_setting['URL Preferences']['extensionsURL'] =  $site_uri . '/wp-content/civicrm/extensions/';
//$civicrm_setting['Extension Preferences']['ext_repo_url'] = false;
...
$civicrm_paths['civicrm.root'] = array(
  'url' => $site_uri . '/wp-content/plugins/contrib/civicrm/civicrm/core',
//  'path' => $repo_root . '/wp-content/plugins/contrib/civicrm/civicrm',
  'path' => $repo_root . '/vendor/civicrm/civicrm-core',
);

$civicrm_paths['civicrm.files'] = array(
  'url' => $site_uri . '/wp-content/uploads/civicrm',
  'path' => $repo_root . '/web/wp-content/uploads/civicrm',
);
...
$civicrm_paths['civicrm.log'] = [
  'path' => $repo_root . '/web/wp-content/uploads/civicrm/ConfigAndLog/',
];

define('CIVICRM_CONFIGLOGDIR', $repo_root . '/web/wp-content/uploads/civicrm/ConfigAndLog/');
